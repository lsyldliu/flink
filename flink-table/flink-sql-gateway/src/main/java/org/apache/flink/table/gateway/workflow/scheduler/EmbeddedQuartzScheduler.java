...
/**
 * An embedded in-memory workflow scheduler based on quartz {@link Scheduler} that store all
 * workflow in memory, it does not have high availability. This service will be embedded in {@link
...
public class EmbeddedQuartzScheduler {
    ...
    public void suspendScheduleWorkflow(String workflowName, String workflowGroup)
        ...
        try {
            if (!quartzScheduler.checkExists(jobKey)) {
                LOG.error("Failed to suspend a non-existent quartz schedule job: {}.", jobKey);
                throw new SchedulerException(
                        String.format(
                                "Failed to suspend a non-existent quartz schedule job: %s.",
                                jobKey));
            }
    ...
    public void resumeScheduleWorkflow(String workflowName, String workflowGroup)
        ...
        try {
            if (!quartzScheduler.checkExists(jobKey)) {
                LOG.error("Failed to resume a non-existent quartz schedule job: {}.", jobKey);
                throw new SchedulerException(
                        String.format(
                                "Failed to resume a non-existent quartz schedule job: %s.",
                                jobKey));
            }
    ...
    public void deleteScheduleWorkflow(String workflowName, String workflowGroup)
        ...
        try {
            if (!quartzScheduler.checkExists(jobKey)) {
                LOG.error("Failed to delete a non-existent quartz schedule job: {}.", jobKey);
                throw new SchedulerException(
                        String.format(
                                "Failed to delete a non-existent quartz schedule job: %s.",
                                jobKey));
            }
