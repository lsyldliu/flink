...
public class CronUtilsExamples {
    ...
    /**
     * This method will walk you through how to create cron definitions: by using pre-defined ones
    ...
    private static void howToWithCronDefinitions() {
        ...
        System.out.println(
                String.format(
                        "Are those definitions the same? %s",
                        cronDefinition1.equals(cronDefinition2)));
    ...
    private static void howToWithCronParser() {
        ...
        System.out.println(
                String.format(
                        "Was the parsing correct? Lets turn it to string again! original: '%s' parsed: '%s'",
        ...
        System.out.println(
                String.format("Are those the same? %s", multicron.equals(cron.asString())));
    }
    ...
    private static void howToBuildAndMigrateCronExpressions() {
        ...
        Cron quartzBuiltCronExpression =
                CronBuilder.cron(CronDefinitionBuilder.instanceDefinitionFor(QUARTZ))
                        .withYear(always())
                        .withDoM(between(1, 3))
                        .withMonth(always())
        ...
        // Ex.: we had everything in Linux, and now we provide a Quartz based scheduling service
        // with a nice REST API.
        // How can we perform the conversions, to remain equivalent while minimizing work?
        // cron-utils provides a CronMapper for that: we can also migrate from/to any other cron
        // format
        String fromQuartzToUnixString =
                CronMapper.fromQuartzToUnix().map(quartzBuiltCronExpression).asString();
        String fromUnixToQuartzString =
                CronMapper.fromUnixToQuartz().map(unixBuiltCronExpression).asString();

        // Lets compare this expressions
        System.out.println(
                String.format(
                        "Original Quartz cron expression: '%s' Mapped from Unix: '%s'",
                        quartzBuiltCronExpressionString, fromUnixToQuartzString));
        System.out.println(
                String.format(
                        "Original Unix cron expression: '%s' Mapped from Quartz: '%s'",
                        unixBuiltCronExpressionString, fromQuartzToUnixString));

        // If we are not performing a migration, but just need to check if expressions are
        // equivalent ...
        System.out.println(
                String.format(
                        "Are both expressions (Quartz: '%s' vs. Unix: '%s') equivalent? %s",
                        quartzBuiltCronExpressionString,
                        unixBuiltCronExpressionString,
                        quartzBuiltCronExpression.equivalent(
                                CronMapper.fromUnixToQuartz(), unixBuiltCronExpression)));
    }
    ...
    private static void howToGetExecutionTimes() {
        ...
        System.out.println(
                String.format(
                        ...
                        now,
                        executionTime.lastExecution(now).get()));
        System.out.println(
                String.format(
                        "Given the Quartz cron '%s' and reference date '%s', next execution will be '%s'",
                        parsedQuartzCronExpression.asString(),
                        now,
                        executionTime.nextExecution(now).get()));
        ...
        System.out.println(
                String.format(
                        ...
                        parsedQuartzCronExpression.asString(),
                        now,
                        executionTime.timeFromLastExecution(now).get().getSeconds()));
        System.out.println(
                String.format(
                        "Given the Quartz cron '%s' and reference date '%s', next execution will be in %s seconds",
                        parsedQuartzCronExpression.asString(),
                        now,
                        executionTime.timeToNextExecution(now).get().getSeconds()));
    }
    ...
    private static void howToHumanReadableDescriptions() {
        ...
        CronDescriptor descriptor =
                CronDescriptor.instance(Locale.US); // we support multiple languages! Just pick one!
        String quartzBuiltCronExpressionDescription = descriptor.describe(parser.parse(expression));
        System.out.println(
                String.format(
                        "Quartz expression '%s' is described as '%s'",
                        expression, quartzBuiltCronExpressionDescription));
    }
}
