...
public class TpcdsPlanTest extends BatchTestBase {
    // protected final BatchTableTestUtil util = batchTestUtil(new TableConfig());

    @Parameterized.Parameter public String caseName;
    protected TableEnvironment tEnv = tEnv();

    @Before
    public void before() throws Exception {
        // config Optimizer parameters
        tEnv.getConfig().getConfiguration().setInteger(CoreOptions.DEFAULT_PARALLELISM, 1500);
        tEnv.getConfig()
                .getConfiguration()
                .setLong(
